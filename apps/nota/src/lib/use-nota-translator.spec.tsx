...
import { useNotaTranslator } from './use-nota-translator';
...
describe('useNotaTranslator', () => {
  it('translates through the locale the reader picked', () => {
    ...
    // Act
    const { result } = renderHook(() => useNotaTranslator());
  ...
  it('keeps the same translator until the locale changes', () => {
    ...
    const { result, rerender } = renderHook(() => useNotaTranslator());
