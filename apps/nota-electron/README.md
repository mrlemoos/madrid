...
2. Run Electron, from the monorepo root:
   ...
   ```bash
   pnpm exec nx dev @getmadrid/nota-electron
   ```
...
```bash
pnpm exec nx run @getmadrid/nota-electron:electron:pack
```
...
`electron-builder` packages the Electron shell only (`dist/**/*` + icons). The packaged app loads **`https://app.getmadrid.app`** (Vercel); it does not embed the Next app. Output is under `apps/nota-electron/release/` (DMG and ZIP per architecture). **macOS** is required for the current `electron-builder.yml` targets.
...
```bash
pnpm exec nx run @getmadrid/nota-electron:electron:release
```

Optional: bump `apps/nota-electron/package.json` for this run only (same as CI’s **`pnpm pkg set version=…`**):

```bash
pnpm exec nx run @getmadrid/nota-electron:electron:release -- --version 1.2.3
```
...
- **CI**: `.github/workflows/release-electron.yml` runs on **`v*`** tags and on **`workflow_dispatch`** (semver + **release kind**: production vs release candidate / draft). It syncs `apps/nota-electron/package.json` version, then runs **`pnpm exec nx run @getmadrid/nota-electron:electron:release`** (build + **`electron-builder --publish always`** via [`tools/electron-github-release.mjs`](../../tools/electron-github-release.mjs)). Actions sets **`GH_TOKEN`** from **`GITHUB_TOKEN`** to upload assets and `latest-mac.yml`.
...
**Local draft publish:** `pnpm exec nx run @getmadrid/nota-electron:electron:release -- --draft` (forwards **`--draft`** to [`tools/electron-github-release.mjs`](../../tools/electron-github-release.mjs)).
...
- **Prod mode (packaged)**: Loads **`https://app.getmadrid.app`** — the same deployed **`nota`** app as the web client ([`src/app-load-url.ts`](src/app-load-url.ts)). The DMG/ZIP contain the Electron shell only; no Next/`nota/dist` embed.
...
- **Madrid Pro entitlement**: The hosted app must have billing / entitlement configured on Vercel (and related server secrets). Ensure CORS allowlists include **`https://app.getmadrid.app`** when you use an explicit list.
- **Clerk session tokens in Electron**: Packaged builds load **`https://app.getmadrid.app`** and call the Clerk Frontend API on **`https://clerk.getmadrid.app`**. If Chromium logs a CORS error on `/v1/client/sessions/.../tokens`, confirm the Clerk **custom domain** DNS record is **DNS only** (not proxied through Cloudflare) per [Clerk production DNS](https://clerk.com/docs/guides/development/deployment/production). The shell also patches missing `Access-Control-Allow-Origin` on Clerk FAPI responses via [`clerk-fapi-cors.ts`](src/clerk-fapi-cors.ts) (`session.webRequest.onHeadersReceived`).
