...
- [x] core imports no web-only package; ui depends on core + `@getmadrid/note-runtime` + `@getmadrid/web-design/*`
...
- Verified during ticket 14's audit: `packages/note-journal-core/src` holds the date-title
  ...
  `apps/nota/src/routes/notes.journal.tsx` composes `@getmadrid/note-journal-ui`; no journal
  files remain under `apps/nota/src/lib` or `apps/nota/src/components`.
- `nx run-many -t build,lint,test` is green for `@getmadrid/note-journal-core`,
  `@getmadrid/note-journal-ui`, and consumers (see ticket 14 Comments).
