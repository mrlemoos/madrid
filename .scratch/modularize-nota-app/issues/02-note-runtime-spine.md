# 02 — Extract `@getmadrid/note-runtime` spine package

**What to build:** A buildable lib `@getmadrid/note-runtime` holding the app-wide runtime spine:
...
`@getmadrid/data-source`. After this ticket every consumer imports `useNotesData*`,
`useNotaPreferences`, and the other runtime hooks from `@getmadrid/note-runtime`, the app root
composes the providers, and the workspace builds green.

**Blocked by:** 01 — `@getmadrid/data-source`.

**Status:** done

- [x] Buildable lib per the house pattern; tagged `scope:getmadrid`, `type:feature`, `platform:web`
- [x] Depends on `@getmadrid/data-source`; boundaries lint passes
...
- `nx run-many -t build,lint,test` is green for `@getmadrid/note-runtime` and its consumers (see
