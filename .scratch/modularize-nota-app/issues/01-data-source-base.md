# 01 — Extract `@nota/data-source` base package

**What to build:** A buildable lib `@nota/data-source` that owns the app's data-access
...
helpers. After this ticket the SPA imports every one of these from `@nota/data-source`
...
- [x] New buildable lib mirrors `packages/note-link-graph` (tsconfig.lib, vite config, `build` target, subpath `exports` with the `@nota/source` condition)
- [x] Tagged `scope:nota`, `type:util`, `platform:web`; boundaries lint passes
...
- [x] Every consumer import rewritten to `@nota/data-source`; `nx run-many -t build lint test` green
...
- `nx run-many -t build,lint,test` is green for `@nota/data-source` and its consumers (see
