# 03 — Extract `@nota/note-doc-plain-text` (shared) and drop the marketing alias

**What to build:** A standalone `platform:shared` buildable lib `@nota/note-doc-plain-text`
...
- [x] Buildable lib per the house pattern; tagged `scope:nota`, `type:util`, `platform:shared`
...
- [x] `apps/nota` and `apps/nota-marketing` import `@nota/note-doc-plain-text`; the marketing `astro.config` alias is deleted
...
- Verified during ticket 14's audit: `packages/note-doc-plain-text/src` holds
  ...
  `tsconfig.json` reference `@nota/note-doc-plain-text` directly via `workspace:*`.
