...
- [x] ui depends on core + `@getmadrid/note-runtime` + `nota-motion` + `@getmadrid/web-design/*`
...
- **UI move (final):** `notes-shell`, `notes-shell-parts`, `notes-sidebar-list` (+ specs)
  moved from `apps/nota/src/components` into `@getmadrid/notes-chrome-ui`, exported as
...
- **Attachment cycle break:** `pdf-attachment-client`, `note-attachment-signed-url-cache`,
  ...
  `notes-chrome-core` into `@getmadrid/data-source` (`./pdf-attachment-client`,
...
- **Collateral moves out of `apps/nota`:** `clientDeleteNoteById` (+ spec) →
  `@getmadrid/note-folders-ui/delete-note-client` (next to `create-note-client`); `AudioToNoteDock`
  and `useAudioNotePendingDrain` (+ specs) → `@getmadrid/note-capture-ui` (`./audio-to-note-dock`,
  `./use-audio-note-pending-drain`). `cn` and `~/types/database.types` app-local imports
  replaced with `@getmadrid/web-design/utils` and `@getmadrid/database-types`. Added a package-local
...
- **Tooling gaps found during the move:** `@getmadrid/web-design`'s Vite lib build was missing a
  `context-menu` entry in `libEntries` (only `.d.ts` existed, no `dist/context-menu.js`),
  which only surfaced once a package (not the app, which resolves via the `@getmadrid/source`
  Vite condition) imported `@getmadrid/web-design/context-menu` under the default test resolution
...
- Sidebar width already in `@getmadrid/nota-motion-core` (ticket 07).
- Verified green: `pnpm exec nx run-many -t build,lint,test -p @getmadrid/notes-chrome-ui,@getmadrid/notes-chrome-core,@getmadrid/note-folders-ui,@getmadrid/note-capture-ui,@getmadrid/nota` and a full `pnpm exec nx run-many -t build,lint,test` sweep, aside from two pre-existing, unrelated failures out of scope: `@getmadrid/web-design:lint` (long-standing promise-handling lint errors in `src/icons/*`, untouched by this ticket) and `@getmadrid/nota-mobile:build` (missing local `eas` CLI).
