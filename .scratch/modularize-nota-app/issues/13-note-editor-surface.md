...
- [x] ui depends on core + `@nota/note-runtime` + `@nota/data-source` + `@nota/editor` + `@nota/web-design/*`
...
extractions (backlink math in `@nota/note-link-graph`, layout/theme in `@nota/note-editor-settings`
/ `@nota/editor`, the typewriter guard in `@nota/nota-motion-core`, save/merge/detail-fetch in
`@nota/data-source`). So `note-editor-core` ended up thin by design — it only holds the one bit of
glue that hadn't been extracted yet: the sidebar/backlinks hash-href builder.

### `@nota/note-editor-core` (`platform:web`)

- `platform:web`, not `platform:shared`, because its only export (`noteHashHref`) wraps
  `hashForScreen` from `@nota/app-navigation-core`, which is itself `platform:web` (module
...
### `@nota/note-editor-ui` (`platform:web`)
...
- `apps/nota/src/lib/notes-yjs/` moved in full (`note-yjs-doc`, `supabase-yjs-provider`,
  ...
  its own deps (`@nota/data-source`, `@nota/notes-yjs-core`, `yjs`, `y-indexeddb`,
  `@nota/database-types`) are all portable into the package. Only fix needed was swapping the
  app's `~/types/database.types` re-export for `@nota/database-types` directly.
- `apps/nota/src/lib/note-title.ts` deleted; `note-editor.tsx` now imports `persistedDisplayTitle`
  from `@nota/data-source/note-title` (the characterisation spec already living there per the
...
- `use-nota-translator.ts` stayed in the app (still used by other app components); the package
  got its own local `use-note-editor-translator.ts` wrapping `@nota/i18n`, matching the
...
- `note-detail-panel.tsx`'s `postSearchIndexNote` call now hits `@nota/nota-server-client`
  directly (base URL + `getClerkAccessToken` from `@nota/data-source/clerk-token-ref`), same
  shape as `ReleaseNotesDialog` in `electron-bridge-ui`, rather than depending on the app's
  `lib/nota-server-client.ts` wrapper.
- `cn` now comes straight from `@nota/web-design/utils`; `hrefForNote`/`parseNoteLinkPath` from
  `@nota/internal-note-link` directly. This left `apps/nota/src/lib/internal-note-link.ts` (a
  thin re-export) with zero importers, so it was deleted too.
- `Note`/`NoteAttachment`/`Database`/`Json` types now come from `@nota/database-types` instead
...
- `vitest.setup.ts` gained the same `window.matchMedia` stub apps/nota uses, needed by
  `note-image-lightbox.tsx` → `@nota/nota-motion-ui/use-prefers-reduced-motion`.

### Rewiring

- `notes-shell.tsx` imports `NoteDetailPanel` from `@nota/note-editor-ui/note-detail-panel`.
- `notes-sidebar-list.tsx` imports `noteHashHref` from `@nota/note-editor-core/note-hash-href`.
- `apps/nota/package.json` gained `@nota/note-editor-core` + `@nota/note-editor-ui`;
...
`pnpm exec nx run-many -t build,lint,test -p @nota/note-editor-core,@nota/note-editor-ui,@nota/data-source,@nota/nota --outputStyle=static`
