...
import HistoryCircleIcon from './history-circle-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('HistoryCircleIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <HistoryCircleIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<HistoryCircleIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<HistoryCircleIcon ref={ref} />);
