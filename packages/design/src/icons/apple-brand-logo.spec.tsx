...
import AppleBrandLogo from './apple-brand-logo.js';
import type { AnimatedIconHandle } from './types.js';

describe('AppleBrandLogo', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <AppleBrandLogo size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<AppleBrandLogo />);
