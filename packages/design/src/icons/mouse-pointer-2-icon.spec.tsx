...
import MousePointer2Icon from './mouse-pointer-2-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('MousePointer2Icon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <MousePointer2Icon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<MousePointer2Icon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<MousePointer2Icon ref={ref} />);
