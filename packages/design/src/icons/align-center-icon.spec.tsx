...
import AlignCenterIcon from './align-center-icon.js';
...
describe('AlignCenterIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    ...
    const { container } = render(
      <AlignCenterIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<AlignCenterIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<AlignCenterIcon ref={ref} />);
