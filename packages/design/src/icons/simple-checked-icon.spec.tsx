...
import SimpleCheckedIcon from './simple-checked-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('SimpleCheckedIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <SimpleCheckedIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<SimpleCheckedIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<SimpleCheckedIcon ref={ref} />);
