...
import ArrowNarrowDownIcon from './arrow-narrow-down-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('ArrowNarrowDownIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <ArrowNarrowDownIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<ArrowNarrowDownIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<ArrowNarrowDownIcon ref={ref} />);
