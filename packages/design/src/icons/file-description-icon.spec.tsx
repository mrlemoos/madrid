...
import FileDescriptionIcon from './file-description-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('FileDescriptionIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <FileDescriptionIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<FileDescriptionIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<FileDescriptionIcon ref={ref} />);
