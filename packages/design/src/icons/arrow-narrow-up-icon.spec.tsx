...
import ArrowNarrowUpIcon from './arrow-narrow-up-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('ArrowNarrowUpIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <ArrowNarrowUpIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<ArrowNarrowUpIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<ArrowNarrowUpIcon ref={ref} />);
