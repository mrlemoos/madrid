...
import ArrowNarrowLeftIcon from './arrow-narrow-left-icon.js';
import type { AnimatedIconHandle } from './types.js';

describe('ArrowNarrowLeftIcon', () => {
  it('renders one svg sized and coloured by its props', () => {
    // Arrange|Act
    const { container } = render(
      <ArrowNarrowLeftIcon size={18} color="rebeccapurple" strokeWidth={1.5} />,
    );
  ...
  it('falls back to a 24px currentColor icon', () => {
    // Arrange|Act
    const { container } = render(<ArrowNarrowLeftIcon />);
  ...
  it('exposes start and stop animation through its ref', () => {
    ...
    // Act
    render(<ArrowNarrowLeftIcon ref={ref} />);
